...
import type { RepoRepositoryOutput, TypesLabel, TypesLabelValue } from 'services/code'
import { useAppContext } from 'AppContext'
import { useGetSpaceParam } from 'hooks/useGetSpaceParam'
import { useUpdateQueryParams } from 'hooks/useUpdateQueryParams'
import { LabelTitle, LabelValuesList, SpaceLabelValuesList } from 'components/Label/Label'
...
const LabelsListing = (props: { activeTab: string; repoMetadata: RepoRepositoryOutput | undefined }) => {
  ...
  const {
    ...
    path: `/api/v1/repos/${repoMetadata?.path}/+/labels`,
    queryParams: {
  ...
  const renderRowSubComponent = React.useCallback(({ row }: { row: Row<LabelType> }) => {
    if (row.original.scope === 0)
      return (
        <LabelValuesList name={row.original?.key as string} scope={row.original?.scope} repoMetadata={repoMetadata} />
      )
    else return <SpaceLabelValuesList name={row.original?.key as string} spaceRef={space} />
  }, [])
  ...
  const columns: Column<LabelType>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        width: '30%',
        Cell: ({ row }: CellProps<LabelType>) => {
          const { message, icon } = getLabelScope(row.original.scope, standalone, repoMetadata, space)
          return (
      ...
      },
      {
        ...
        Cell: ({ row }: CellProps<LabelType>) => {
          ...
          const { mutate: deleteLabel } = useMutate({
            verb: 'DELETE',
            path: `/api/v1/repos/${repoMetadata?.path}/+/labels/${encodeURIComponent(encodedLabelKey)}`
          })
